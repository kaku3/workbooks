...
何か作りたいモノがある時に、AIに「いい感じに」というとそこそこいい感じに作ってくれる。

ただ、それなりに「思った通り」にしようとした場合、思ったことを詳細に伝える必要が出てくる。

ヒトに対して頼む時は、要件定義→外部設計→詳細設計→開発といった手順を取る事が多いが、その手順を取るの大変である。

本プロジェクトでは「チケット管理システム(trak)」の要件定義書だけを作成し、外部設計以後のタスクはAIに実施させ、ヒトは成果物の確認を行うという方針で進めることとする。
