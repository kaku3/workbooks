...
import { loadSortData, updateTicketOrder } from "./ticket-sort";
...
export const createTicket = async (
  ...
  await fs.writeFile(path.join(ticketsDir, `${ticketId}.md`), data.content);
  ...
  // 既存の全チケットのソート順を取得して、新しいチケットを一番上に配置
  const sortData = await loadSortData();
  const entries = Object.entries(sortData);

  // 既存のチケットのソート順を更新
  for (const [id, order] of entries) {
    await updateTicketOrder(id, order + 1);
  }

  // 新しいチケットを一番上（0）に設定
  await updateTicketOrder(ticketId, 0);
