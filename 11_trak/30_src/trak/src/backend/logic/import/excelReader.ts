 ...
 * チケット情報の読み込み
 */
export async function readTicketsFromExcel(
  file: ArrayBuffer
): Promise<RawTicketData[]> {
  const workbook = XLSX.read(file, { type: "array" });
  const sheet = workbook.Sheets["WBS"];
  ...
  return data
    .filter(
      (row): row is Required<Pick<ExcelRow, "title" | "status">> & ExcelRow =>
    ...
    .map((row) => ({
      ...
      tags: [
        row.tag1,
        ...(row.tag2 ? row.tag2.split(";").map((tag) => tag.trim()) : []),
