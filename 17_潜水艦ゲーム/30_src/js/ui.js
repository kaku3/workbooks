...
function _showSonarAlert() {
  const el = document.getElementById('sonar-alert-overlay');
  if (!el) return;
  // アニメーションを再発火させるためクラスを一度外してから付け直す
  el.classList.remove('hidden', 'sonar-alert-show');
  void el.offsetWidth; // reflow
  el.classList.add('sonar-alert-show');
  // アニメーション終了後に非表示へ
  setTimeout(() => {
    el.classList.remove('sonar-alert-show');
    el.classList.add('hidden');
  }, 2000);
}
